...
import { TorchLoginGate } from '@/components/torch-login/gate'
import { TorchMarket } from '@/components/torch-market'
import { TorchModelMenu } from '@/components/torch-model-menu'
import { useMediaQuery } from '@/hooks/use-media-query'
import { isFocusWithin } from '@/lib/keybinds/combo'
import { cn } from '@/lib/utils'
import { $torchLogin } from '@/store/torch-login'
import { applyTorchModel } from '@/store/torch-models'
...
export function DesktopController() {
  ...
  const torchSession = useStore($torchLogin).session
  ...
  // Torch-native model switch: persist the pick as the profile default (routed
  // through the metering proxy) and switch the live session to it.
  const selectTorchModel = useCallback(
    async (model: string) => {
      await applyTorchModel(model)
      await selectModel({ model, provider: 'custom' })
    },
    [selectModel]
  )

  const modelMenuContent = useMemo(
    () => (gatewayState === 'open' ? <TorchModelMenu onSelect={model => void selectTorchModel(model)} /> : null),
    [gatewayState, selectTorchModel]
  )
  ...
  const overlays = (
    <>
      ...
      {!isSecondaryWindow() && <TorchMarket />}
      {!isSecondaryWindow() && <DesktopInstallOverlay />}
      {!isSecondaryWindow() && (
        <DesktopOnboardingOverlay
          enabled={gatewayState === 'open' && Boolean(torchSession)}
          onCompleted={() => {
            void refreshHermesConfig()
            void refreshCurrentModel()
            void queryClient.invalidateQueries({ queryKey: ['model-options'] })
      ...
      <PaneMain>
        <Routes>
          ...
          <Route
            element={
              <Suspense fallback={null}>
                <CliView setStatusbarItemGroup={setStatusbarItemGroup} />
              </Suspense>
            }
            path="cli"
          />
