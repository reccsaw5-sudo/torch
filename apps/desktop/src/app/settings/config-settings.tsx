...
import { TorchModelSettings } from './torch-model-settings'
...
export function ConfigSettings({
  ...
  // The Torch model page is self-contained (metering-proxy catalog + reasoning
  // default); the stock model-section config fields (context length / fallback
  // providers) reference the upstream provider universe and don't apply, so
  // they're suppressed here.
  if (activeSectionId === 'model') {
    return (
      <SettingsContent>
        <div className="mb-6">
          <TorchModelSettings onMainModelChanged={onMainModelChanged} />
        </div>
        <input
