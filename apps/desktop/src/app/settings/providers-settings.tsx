...
// White-label: the client only exposes the Torch metering provider here. All
// upstream providers and their base URLs are configured server-side in the
// admin console, so the API-keys list is filtered down to Torch only.
const ONLY_PROVIDER = 'torch'
...
function buildProviderKeyGroups(vars: Record<string, EnvVarInfo>): ProviderKeyGroup[] {
  ...
  for (const [key, info] of Object.entries(vars)) {
    ...
    // White-label: only surface the Torch provider (match by label/id or the
    // TORCH_ env prefix); everything else is managed in the admin console.
    if (name.toLowerCase() !== ONLY_PROVIDER && !key.toUpperCase().startsWith('TORCH_')) {
      continue
    }
