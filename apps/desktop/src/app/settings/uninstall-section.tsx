...
export function UninstallSection() {
  const { t } = useI18n()
  const u = t.settings.about.uninstall
  ...
  const OPTIONS: ModeOption[] = [
    { mode: 'gui', title: u.guiTitle, description: u.guiDesc, consequence: u.guiConsequence, needsAgent: false },
    { mode: 'lite', title: u.liteTitle, description: u.liteDesc, consequence: u.liteConsequence, needsAgent: true },
    { mode: 'full', title: u.fullTitle, description: u.fullDesc, consequence: u.fullConsequence, needsAgent: true }
  ]
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        setError(result.message || result.error || u.cannotStart)
        setRunning(false)
        setPending(null)
      }
      ...
      <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3">
        {loading ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{u.confirmConsequence(pendingOption.consequence)}</p>
