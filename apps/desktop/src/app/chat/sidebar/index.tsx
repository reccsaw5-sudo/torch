...
const SIDEBAR_NAV: SidebarNavItem[] = [
  ...
  },
  {
    ...
    children: [
      { id: 'skills', label: '', icon: props => <Codicon name="symbol-misc" {...props} />, route: SKILLS_ROUTE },
      { id: 'memory', label: '', icon: props => <Codicon name="database" {...props} />, route: MEMORY_ROUTE },
      { id: 'cron', label: '', icon: props => <Codicon name="watch" {...props} />, route: CRON_ROUTE },
      { id: 'mcp', label: '', icon: props => <Codicon name="server" {...props} />, route: `${SKILLS_ROUTE}?tab=mcp` },
      { id: 'cli', label: '', icon: props => <Codicon name="terminal" {...props} />, route: CLI_ROUTE },
...
// no-drag: nav rows sit directly under the titlebar's [-webkit-app-region:drag]
// strips (app-shell.tsx) with only 6px of clearance. Drag regions win
// hit-testing over DOM, so the carve-out keeps clicks from being swallowed.
...
function navActive(item: SidebarNavItem, currentView: AppView, routeSearch: string): boolean {
  switch (item.id) {
    case 'agents':
      return currentView === 'agents'

    case 'artifacts':
      return currentView === 'artifacts'

    case 'cli':
      return currentView === 'cli'
...
function NavRowButton({
  ...
  const isInteractive = Boolean(item.action) || Boolean(item.route) || Boolean(item.children)
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-px">
              {SIDEBAR_NAV.map(item => {
                if (item.children) {
                  const anyChildActive = item.children.some(child => navActive(child, currentView, routeSearch))

                  return (
                    <SidebarMenuItem key={item.id}>
                      <NavRowButton
                        active={anyChildActive && !toolsOpen}
                      ...
                      />
                      {contentVisible && toolsOpen && (
                        <div className="mt-px ml-[1.125rem] flex flex-col gap-px border-l border-(--ui-stroke-tertiary) pl-1.5">
                          {item.children.map(child => (
                            <SidebarMenuItem key={child.id}>
                              <NavRowButton
                                active={navActive(child, currentView, routeSearch)}
                                contentVisible={contentVisible}
                                isChild
                                item={child}
                                label={s.nav[child.id] ?? child.label}
                ...
                return (
                  <SidebarMenuItem key={item.id}>
                    <NavRowButton
                      active={navActive(item, currentView, routeSearch)}
                      contentVisible={contentVisible}
                      isChild={false}
                      item={item}
                      label={s.nav[item.id] ?? item.label}
