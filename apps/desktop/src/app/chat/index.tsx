...
export function ChatView({
  ...
  // Torch selects models natively via TorchModelMenu (`/v1/models` from the
  // metering proxy), so the Hermes ~40-provider `model.options` universe is
  // unused here — its only consumer (`quickModels`) is never rendered.
  // Building that payload (`pricing`+`capabilities`+`include_unconfigured`) is
  // CPU-heavy and, on the single-worker backend, stalls concurrent
  // `/api/config`* reads — making the Settings pages load slowly. Keep it off.
