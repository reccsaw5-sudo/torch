...
import { $translateOn, translateStrings, useTr } from '@/lib/torch-translate'
...
function skillSubtitle(skill: SkillInfo, tr: (text: string | null | undefined) => string): React.ReactNode {
  const category = tr(prettyName(categoryFor(skill)))
...
export function SkillsView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: SkillsViewProps) {
  const { t } = useI18n()
  const tr = useTr()
  const translateOn = useStore($translateOn)
  ...
  // When translation is on, batch-translate the visible catalog text (skill
  // descriptions/categories + toolset descriptions/labels). Already-cached
  // strings are skipped, so re-runs on data change are cheap.
  useEffect(() => {
    if (!translateOn) {
      return
    }
    ...
    for (const skill of skills ?? []) {
      texts.push(asText(skill.description), prettyName(categoryFor(skill)))
    }

    for (const toolset of toolsets ?? []) {
      texts.push(asText(toolset.description), toolsetDisplayLabel(toolset))
    }

    if (texts.length > 0) {
      void translateStrings(texts)
    }
  }, [translateOn, skills, toolsets])
  ...
  return (
    <PageSearchShell
      ...
      searchTrailingAction={
        <div className="flex items-center gap-1.5">
          {mode === 'skills' && (
            <Button className="h-8 gap-1.5 rounded-lg" onClick={() => setCreateOpen(true)} size="sm">
    ...
    >
            ...
            >
              {visibleSkills.map(skill => (
                <CapRow
                  ...
                  subtitle={skillSubtitle(skill, tr)}
                  title={skill.name}
            ...
            {visibleToolsets.map(toolset => {
              ...
              return (
                <CapRow
                  ...
                  subtitle={tr(asText(toolset.description))}
                  title={tr(label)}
      ...
      <CreateSkillDialog
        onCreated={() => void queryClient.invalidateQueries({ queryKey: SKILLS_QUERY_KEY })}
...
function SkillDetail({ onArchive, onEdit, skill }: { onArchive: () => void; onEdit: () => void; skill: SkillInfo }) {
  ...
  return (
    <>
      <DetailHeader
        description={tr(asText(skill.description)) || t.skills.noDescription}
        pills={
          <>
            <PanelPill>{tr(prettyName(categoryFor(skill)))}</PanelPill>
...
function ToolsetDetail({
  ...
  return (
    <>
      ...
      <DetailHeader
        description={tr(asText(toolset.description)) || t.skills.noDescription}
