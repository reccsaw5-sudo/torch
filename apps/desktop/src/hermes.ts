...
// Create a custom skill from a single SKILL.md payload (name + frontmatter'd
// content). Same validated write path as the agent's skill_manage tool.
export function createSkill(name: string, content: string, category?: string): Promise<SkillMutationResponse> {
  return window.hermesDesktop.api<SkillMutationResponse>({
    ...profileScoped(),
    path: '/api/skills',
    method: 'POST',
    body: { name, content, category: category?.trim() ? category.trim() : null }
  })
}
